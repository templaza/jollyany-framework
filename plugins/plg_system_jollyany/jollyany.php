...
class plgSystemJollyany extends CMSPlugin {
    ...
    public function onBeforeRender()
    {
        ...
        if (Framework::isSite()) {
            ...
            $document->getWA()->useScript('astroid.uikit');
            $document->getWA()->useScript('astroid.uikit-icons');
    ...
    public function onAfterInitialise() {
        ...
        if (version_compare(Helper\Constants::$astroid_version, "2.5.5")<1) {
            Factory::getApplication()->enqueueMessage( 'Your Astroid version is '.Helper\Constants::$astroid_version.'. Jollyany required Astroid v2.5.5+. Please download and update <a target="_blank" href="https://github.com/templaza/astroid-framework/releases/latest">latest Astroid version</a>' );
        }
        if ($option == 'com_ajax') {
            switch ($jollyany) {
                case 'check_domain':
                    ...
                    try {
                        ...
                        if ( is_object( $license ) && isset( $license->purchase_code ) ) {
                            ...
                            throw new \Exception(Text::_('JOLLYANY_FREE_LICENSE'), 204);
                        }
                    } catch (\Exception $e) {
                        $return["status"] = "error";
                ...
                case 'activation' :
                    ...
                    try {
                        ...
                        File::write(JPATH_ROOT.DIRECTORY_SEPARATOR.'media'.DIRECTORY_SEPARATOR.'jollyanykey'.DIRECTORY_SEPARATOR.uniqid('key_').'.txt', JollyanyFrameworkHelper::maybe_serialize($license));
                        ...
                        if ( is_object( $license ) && isset( $license->purchase_code ) ) {
                            ...
                            if (is_array($templates) && isset($templates[$install_code])) {
                                ...
                                if (!$this->compareJoomlaVersion($template['joomla_version'])) {
                                    throw new \Exception(Text::_('JOLLYANY_AJAX_NOT_COMPATIBLE_JOOMLA_VERSION'));
                                }
                            }

                            if (!$demo_data_package) throw new \Exception(Text::_('JOLLYANY_AJAX_ERROR_NO_FILE_INSTALL'));
                            ...
                            if($response -> code == 200) {
                                ...
                                throw new \Exception(Text::_('JOLLYANY_AJAX_ERROR_CAN_NOT_CONNECT_SERVER'));
                            }
                            $return["status"] = "success";
                            $return["code"] = 200;
                        } else {
                            throw new \Exception(Text::_('JOLLYANY_AJAX_ERROR_NO_LICENSE'));
                        }
                    } catch (\Exception $e) {
                        $return["status"] = "error";
                        $return["code"] = $e->getCode();
                    ...
                    try {
                        ...
                        if ( is_object( $license ) && isset( $license->purchase_code ) ) {
                            ...
                            if ($package['extractdir']== null){
                                throw new \Exception(Text::_('JOLLYANY_AJAX_ERROR_PACKAGE_NOT_FOUND'));
                            } else {
                                if (!$raw_data       = file_get_contents(JPATH_ROOT.DIRECTORY_SEPARATOR.'installation'.DIRECTORY_SEPARATOR.'sql'.DIRECTORY_SEPARATOR.'databases.json')) {
                                    throw new \Exception(Text::_('JOLLYANY_AJAX_ERROR_QUICKSTART_CAN_NOT_FOUND'));
                                }
                    ...
                    try {
                        ...
                        if ( is_object( $license ) && isset( $license->purchase_code ) ) {
                            ...
                            if (is_array($templates) && isset($templates[$install_code])) {
                                ...
                                throw new \Exception(Text::_('JOLLYANY_AJAX_ERROR_INVALID_CODE'));
                            }
                            $return["status"] = "success";
                            $return["code"] = 200;
                        } else {
                ...
                case 'get_extension_package':
                    ...
                    try {
                        ...
                        if ( is_object( $license ) && isset( $license->purchase_code ) ) {
                            $install_code          =   $this->app->input->post->get('install_code', '', 'RAW');
                            if (empty($install_code)) throw new \Exception(Text::_('JOLLYANY_AJAX_ERROR_NO_FILE_INSTALL'));
                ...
                case 'install_package':
                    ...
                    try {
                        ...
                        if ( is_object( $license ) && isset( $license->purchase_code ) ) {
                            ...
                            if (!$extension_package && !$template_package) throw new \Exception(Text::_('JOLLYANY_AJAX_ERROR_NO_FILE_INSTALL'));
                            if (is_object($extension) && isset($extension->name)) {
                                ...
                                if ($extension->type == 'included') {
                                    ...
                                    if (!$url)
                                    {
                                        throw new \Exception(Text::_('JOLLYANY_AJAX_ERROR_EXTENSION_URL_NOT_FOUND'));
                                    }
                                    ...
                                    if (!$p_file)
                                    {
                                        throw new \Exception(Text::_('JOLLYANY_AJAX_ERROR_EXTENSION_URL_INVALID'));
                                    }
                    ...
                    try {
                        ...
                        if ($ext_code) {
                            ...
                            if (!empty($ext_version)) {
                                $return['data']     =   Text::_('JOLLYANY_CURRENT_VERSION'). ': '. $ext_version;
                            ...
                            if (!empty($ext_version)) {
                                ...
                                $return['data']     =    Text::_('JOLLYANY_INSTALLED_VERSION'). ': '. $ext_version;
                ...
                case 'cache_thumb':
                    ...
                    try {
                        ...
                        for ($i = 0; $i < count($thumbs); $i++) {
                            ...
                            if ($response->code == 200) {
                                File::write(JPATH_ADMINISTRATOR.DIRECTORY_SEPARATOR.'cache'.DIRECTORY_SEPARATOR.'jollyany'.$thumbs[$i], $response->body);
                            }
                        }
                ...
                case 'course_contact_form':
                    ...
                    try {
                        ...
                        if (isset($from_name) && $from_name) {
                            $mail_body .= '<p><strong>' . Text::_('JOLLYANY_COURSE_AJAX_CONTACT_NAME'). '</strong>: ' . $from_name .'</p>';
                        }
                        if (isset($from_email) && $from_email) {
                            $mail_body .= '<p><strong>' . Text::_('JOLLYANY_COURSE_AJAX_CONTACT_EMAIL'). '</strong>: ' . $from_email .'</p>';
                        }
                        if (isset($phone) && $phone) {
                            $mail_body .= '<p><strong>' . Text::_('JOLLYANY_COURSE_AJAX_CONTACT_PHONE'). '</strong>: ' . $phone .'</p>';
                        }
                        if (isset($message) && $message) {
                            $mail_body .= '<p><strong>' . Text::_('JOLLYANY_COURSE_AJAX_CONTACT_MESSAGE'). '</strong>: ' . $message .'</p>';
                        }
                        if (isset($agreement) && $agreement) {
                            $mail_body .= '<p><strong>' . Text::_('JOLLYANY_COURSE_AJAX_CONTACT_AGREEMENT'). '</strong>: ' . Text::_('JYES'). '</p>';
                        } else {
                            $mail_body .= '<p><strong>' . Text::_('JOLLYANY_COURSE_AJAX_CONTACT_AGREEMENT'). '</strong>: ' . Text::_('JNO'). '</p>';
                        }
                ...
                case 'course_get_data' :
                    ...
                    try {
                        ...
                        if (!$cid || !$id) throw new \Exception(Text::_('JOLLYANY_AJAX_ERROR'));
                        $course_data    =   JollyanyFrameworkCourse::getData($cid);
                        $modal_content  =   '';
                        if ($course_data) {
                            $courses    =   json_decode($course_data->data,true);
                            if (!$courses[$id]) throw new \Exception(Text::_('JOLLYANY_AJAX_ERROR'));
                            $lesson     =   $courses[$id];
