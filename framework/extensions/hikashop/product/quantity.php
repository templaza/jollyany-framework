...
if($this->global_on_listing && !empty($this->row->formName)) {
	if($add_to_cart) {
?>
	<a class="<?php echo $css_button . ' ' . $css_button_cart; ?>" href="<?php echo hikashop_completeLink('product&task=updatecart&add=1'); ?> jollyany-hikashop-atc" onclick="if(window.hikashop.addToCart) { return window.hikashop.addToCart(this); }" data-addTo-div="<?php echo $this->row->formName; ?>" data-addToCart="list" data-addTo-class="add_in_progress"><span><?php
...
?>
<div class="hikashop_product_stock_count<?php echo $stock_class; ?>">
<?php
...
<!-- ADD TO CART BUTTON -->
<?php
if ($add_to_cart && $add_to_wishlist) echo '<div class="uk-button-group">';
if($add_to_cart) {
?>
	<a class="<?php echo $css_button . ' ' . $css_button_cart; ?> jollyany-hikashop-atc" rel="nofollow" href="<?php echo hikashop_completeLink($classical_url); ?>" onclick="if(window.hikashop.addToCart) { return window.hikashop.addToCart(this); }" data-addToCart="<?php echo $this->row->product_id; ?>" data-addTo-div="hikashop_product_form" data-addTo-class="add_in_progress"><span><?php
...
if($add_to_wishlist) {
?>
	<a class="<?php echo $css_button . ' ' . $css_button_wishlist.(empty($this->row->product_addtowishlist_message) ? ' uk-padding-small uk-padding-remove-vertical' : ''); ?>" rel="nofollow" href="<?php echo hikashop_completeLink($classical_url); ?>" onclick="if(window.hikashop.addToWishlist) { return window.hikashop.addToWishlist(this); }" data-addToWishlist="<?php echo $this->row->product_id; ?>" data-addTo-div="hikashop_product_form" data-addTo-class="add_in_progress"><span><?php
...
if ($add_to_cart && $add_to_wishlist) echo '</div>';
?>
<!-- EO WISHLIST BUTTON -->
