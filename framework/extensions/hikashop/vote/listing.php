...
<div class="hikashop_listing_comment">
    <div data-uk-grid>
        <div class="uk-width-expand"><h5><?php echo JText::_('HIKASHOP_LISTING_COMMENT');?></h5></div>
        <?php
        if($row->vote_comment_sort_frontend && count($this->elts)) {
            $sort = hikaInput::get()->getString('sort_comment','');
            ?>
            <div class="hikashop_sort_listing_comment uk-width-auto">
                <select class="uk-select" name="sort_comment" onchange="refreshCommentSort(this.value); return false;">
...
?>
            <li<?php echo $table_elements; ?> class="hika_comment_listing">
                <article class="uk-comment">
                    <header<?php echo $td_elements; ?> class="uk-comment-header">
                        ...
                        <ul class="uk-comment-meta uk-subnav uk-margin-small-top">
                            <?php
					...
					<span itemprop="ratingValue"><?php echo $nb_star_vote; ?></span>
                            ...
                            <?php } ?>
                            <?php if($row->useful_rating == 1) {
                                if (($elt->total_vote_useful != 0) || ($row->useful_style == 'helpful' && $elt->vote_user_id != hikashop_loadUser() && $elt->vote_user_id != hikashop_getIP())){
                                ?>
                            ...
                            <li>
                                ...
                                <?php } ?>
                                <?php if(($row->hide == 0 && $elt->already_vote == 0 && $elt->vote_user_id != hikashop_loadUser() && $elt->vote_user_id != hikashop_getIP()) || ($row->useful_style == "thumbs")){ ?>
                                <li>
                                    <?php
                                    echo '<div class="uk-grid-small hika-comment-helpful" data-uk-grid>';
                                    if($row->hide == 0 && $elt->already_vote == 0 && $elt->vote_user_id != hikashop_loadUser() && $elt->vote_user_id != hikashop_getIP()){
                                        ...
                                        <?php if($row->useful_style == 'thumbs'){?>
                                            <div class="hika_comment_listing_useful_p"><?php
                                                ...
                                                ?></div>
                                        <?php } ?>
                                        <div class="hika_comment_listing_useless" title="<?php echo JText::_('HIKA_USELESS'); ?>" onclick="if(!window.Oby.hasClass(this, 'next_button_disabled')) hikashop_vote_useful(<?php echo $elt->vote_id;?>,2); window.Oby.addClass(this, 'next_button_disabled');"></div>
                                        <?php
                            ...
                            <?php if(!empty ($elt->purchased)) { ?>
                                <li class="hikashop_vote_listing_useful_bought"><?php echo JText::_('HIKASHOP_VOTE_BOUGHT_COMMENT'); ?></li>
                            <?php } ?>
                        </ul>
                    </header>
                    <div id="<?php echo $i++; ?>"<?php echo $div_elements; ?> class="uk-comment-body hika_comment_listing_content">
                        <?php echo nl2br($this->escape($elt->vote_comment)); ?>
                    </div>
                </article>
            </li>
<?php } ?>
