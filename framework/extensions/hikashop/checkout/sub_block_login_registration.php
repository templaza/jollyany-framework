...
<?php
$labelcolumnclass = 'uk-form-label';
$inputcolumnclass = 'uk-form-controls';
...
<!-- NAME -->
	<div class="hkform-group control-group hikashop_registration_name_line" id="hikashop_registration_name_line">
		...
		<div class="<?php echo $inputcolumnclass;?>">
			<input type="text" name="data[register][name]" id="register_name" value="<?php echo $this->escape($this->mainUser->get( 'name' ));?>" class="uk-input" size="30" maxlength="50"/>
		</div>
	</div>
<!-- EO NAME -->
<!-- USERNAME -->
	<div class="hkform-group control-group hikashop_registration_username_line" id="hikashop_registration_username_line">
		...
		<div class="<?php echo $inputcolumnclass;?>">
			<input type="text" name="data[register][username]" id="register_username" value="<?php echo $this->escape($this->mainUser->get( 'username' ));?>" class="uk-input validate-username" maxlength="25" size="30" />
		</div>
	</div>
<!-- EO USERNAME -->
<?php
...
<!-- EMAIL -->
	<div class="hkform-group control-group hikashop_registration_email_line">
		...
		<div class="<?php echo $inputcolumnclass;?>">
			<input<?php if($this->config->get('show_email_confirmation_field',0)){echo ' autocomplete="off"';} ?> type="text" name="data[register][email]" id="register_email" value="<?php echo $this->escape($this->mainUser->get( 'email' ));?>" class="uk-input validate-email" maxlength="100" size="30" />
		</div>
	</div>
<!-- EO EMAIL -->
<!-- EMAIL CONFIRMATION -->
...
?>
	<div class="hkform-group control-group hikashop_registration_email_confirm_line">
		...
		<div class="<?php echo $inputcolumnclass;?>">
			<input autocomplete="off" type="text" name="data[register][email_confirm]" id="register_email_confirm" value="<?php echo $this->escape($this->mainUser->get('email'));?>" class="uk-input validate-email" maxlength="100" size="30" onchange="if(this.value!=document.getElementById('register_email').value){alert('<?php echo JText::_('THE_CONFIRMATION_EMAIL_DIFFERS_FROM_THE_EMAIL_YOUR_ENTERED',true); ?>'); this.value = '';}" />
		</div>
	</div>
<?php
}
...
?>
	<div class="hkform-group control-group hikashop_registration_password_line" id="hikashop_registration_password_line">
		...
		<div class="<?php echo $inputcolumnclass;?>">
			<input autocomplete="off" type="password" name="data[register][password]" id="register_password" value="" class="uk-input validate-password" size="30" >
		</div>
	</div>
	<div class="hkform-group control-group hikashop_registration_password2_line" id="hikashop_registration_password2_line">
		...
		<div class="<?php echo $inputcolumnclass;?>">
			<input autocomplete="off" type="password" name="data[register][password2]" id="register_password2" value="" class="uk-input validate-password" size="30" >
		</div>
	</div>
<?php
}
...
<?php
		...
		echo $this->fieldsClass->display(
				...
				'data['.$type.']['.$fieldName.']',
				false,
				' class="'.(in_array($this->extraFields[$type][$fieldName]->field_type, array('singledropdown', 'zone')) ? 'uk-select' : 'uk-input').'" '.$onWhat.'="window.hikashop.toggleField(this.value,\''.$fieldName.'\',\''.$type . '_' . $this->step . '_' . $this->module_position.'\',0,\'hikashop_\');"',
				false,
...
<!-- BILLING ADDRESS TITLE -->
    <h4><?php echo JText::_( 'ADDRESS_INFORMATION' ); ?></h4>
...
<?php
		...
		echo $this->fieldsClass->display(
				...
				'data['.$type.']['.$fieldName.']',
				false,
				'class="'.(in_array($this->extraFields[$type][$fieldName]->field_type, array('singledropdown', 'zone')) ? 'uk-select' : 'uk-input').'" '.$onWhat.'="window.hikashop.toggleField(this.value,\''.$fieldName.'\',\'address_shipping_' . $this->step . '_' . $this->module_position.'\',0,\'hikashop_\');"',
				false,
				$this->extraFields[$type],
				@$this->$type,
				false
