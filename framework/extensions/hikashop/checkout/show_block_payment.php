...
?>
<h4><?php echo JText::_('HIKASHOP_PAYMENT_METHOD');?></h4>
<?php
	}
...
?>
        <div class="hikashop_payment_methods">
        <?php
    }
    foreach($cart->usable_methods->payment as $payment) {
...
?>
                ...
                if(!empty($payment->payment_description)) {
                    ?>
                    <div class="hikashop_checkout_payment_description uk-text-meta uk-margin-small-top"><?php
                ...
                if(empty($this->options['read_only']) && !empty($payment->ask_cc)) {
                    ?>
                    <div id="<?php echo $container_id; ?>__card" class="hikashop_checkout_payment_card uk-form-stacked uk-margin-top" style="<?php echo $selected ? '' : ' display:none;'; ?>"><?php
                        $cc_data = $this->checkoutHelper->getCreditCard($payment);
                        if(empty($cc_data))
                            hikashop_loadJsLib('creditcard');
                        ?>
                        <div class="hika_options large uk-margin">
                            <?php
                            if(!empty($payment->ask_owner)) {
                                ?>
                                <div class="uk-margin">
                                    <label class="uk-form-label"><?php echo JText::_('CREDIT_CARD_OWNER'); ?></label>
                                    <div class="uk-form-controls">
                                        <?php
                                        if(empty($cc_data)) {
                                            ?>
                                            <input type="text" class="uk-input uk-form-width-large" autocomplete="off" name="checkout[payment][card][<?php echo $payment->payment_id;?>][owner]" value="" />
                                            <?php
                                        } else {
                                            ?>
                            ...
                            if(!empty($payment->ask_cctype)) {
                                ?>
                                <div class="uk-margin">
                                    <label class="uk-form-label"><?php echo JText::_('CARD_TYPE'); ?></label>
                                    <div class="uk-form-controls"><?php
                                        if(empty($cc_data)) {
                                            $values = array();
                            ...
                            ?>
                            <div class="uk-margin">
                                <label class="uk-form-label" for="hk_co_p_c_n_<?php echo $payment->payment_id; ?>"><?php echo JText::_('CREDIT_CARD_NUMBER'); ?></label>
                                <div class="uk-form-controls">
                                    <?php
                                    if(empty($cc_data)) {
                                        ?>
                                        <input type="text" class="uk-input uk-form-width-large" autocomplete="off" name="checkout[payment][card][<?php echo $payment->payment_id; ?>][num]" value="" onchange="if(!hikashopCheckCreditCard(this.value)){ this.value = ''; }" id="hk_co_p_c_<?php echo $payment->payment_id; ?>"/>
                                        <?php
                                    } else {
                                        ?>
                            ...
                            <div class="uk-inline-block uk-width-small">
                                <label class="uk-form-label" for="hk_co_p_c_e_<?php echo $payment->payment_id; ?>"><?php echo JText::_('EXPIRATION_DATE'); ?></label>
                                <div class="uk-form-controls">
                                    <?php
                                    if(empty($cc_data)) {
                                        ?>
                                        <input type="text" autocomplete="off" name="checkout[payment][card][<?php echo $payment->payment_id; ?>][mm]" class="card_expiration_date_input uk-input uk-form-width-xsmall" maxlength="2" size="2" value="" placeholder="<?php $mm = JText::_('CC_MM'); if($mm=='CC_MM') $mm = JText::_('MM'); echo $mm;?>" id="hk_co_p_c_e_<?php echo $payment->payment_id; ?>"/>
                                        /
                                        <input type="text" autocomplete="off" name="checkout[payment][card][<?php echo $payment->payment_id; ?>][yy]" class="card_expiration_date_input uk-input uk-form-width-xsmall" maxlength="2" size="2" value="" placeholder="<?php echo JText::_('YY');?>" />
                                        <?php
                                    } else {
                                        ?>
                            ...
                            if(!empty($payment->ask_ccv)) {
                                ...
                                ?>
                                <div class="uk-inline-block uk-width-small">
                                    <label class="uk-form-label" for="hk_co_p_c_v_<?php echo $payment->payment_id; ?>" data-toggle="hk-tooltip" data-title="<?php echo htmlspecialchars('<strong>'.JText::_('CVC_TOOLTIP_TITLE').'</strong><br/>'.JText::_('CVC_TOOLTIP_TEXT'), ENT_COMPAT, 'UTF-8'); ?>"><?php echo JText::_('CARD_VALIDATION_CODE'); ?></label>
                                    <div class="uk-form-controls">
                                        <?php
                                        if(empty($cc_data)) {
                                            ?>
                                            <input class="uk-input uk-form-width-small" type="text" autocomplete="off" name="checkout[payment][card][<?php echo $payment->payment_id;?>][ccv]" maxlength="4" size="4" value="" id="hk_co_p_c_v_<?php echo $payment->payment_id; ?>"/>
                                            <?php
                                        } else {
                                            ?>
                        ...
                        if(empty($cc_data)) {
                            ...
                            <div class="hikashop_checkout_payment_submit">
                                <button class="<?php echo $this->config->get('css_button','hikabtn'); ?> uk-button uk-button-default hikabtn_checkout_payment_submit" onclick="return window.checkout.submitPayment(<?php echo $this->step; ?>,<?php echo $this->module_position; ?>);"><?php echo JText::_('HIKA_SUBMIT'); ?></button>
                            </div>
                            <?php
                        } else {
                            ?>
                            <div class="hikashop_checkout_payment_submit">
                                <button class="<?php echo $this->config->get('css_button','hikabtn'); ?> uk-button uk-button-default hikabtn_checkout_payment_reset" onclick="return window.checkout.resetPayment(<?php echo $this->step; ?>,<?php echo $this->module_position; ?>,<?php echo $payment->payment_id; ?>);"><?php echo JText::_('RESET'); ?></button>
                            </div>
                            <?php
                        }
                        ?>
